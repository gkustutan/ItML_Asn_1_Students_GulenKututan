...
class edaDF:
    ...
    def correlationHeatmap(self):
        correlation_matrix = self.data[self.num].corr()
        plt.figure(figsize=(12, 8))
        sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', fmt=".2f", linewidths=.5)
        plt.title('Correlation Heatmap')
        plt.show()

    def boxPlots(self, splitTarg=False, show=True):
        n = len(self.num)
        cols = 2
        figure, ax = plt.subplots(math.ceil(n / cols), cols)
        r = 0
        ...
        for col in self.num:
            if splitTarg == False:
                sns.boxplot(data=self.data, x=self.target, y=col, ax=ax[r][c])
            if splitTarg == True:
                sns.boxplot(data=self.data, x=col, y=self.target, ax=ax[r][c])
            c += 1
            if c == cols:
                r += 1
                c = 0
    ...
    def pairPlot(self, hue=None):
        sns.pairplot(self.data[self.num], hue=hue)
    ...
    def targetDistribution(self):
        ...
        sns.countplot(data=self.data, x=self.target)
        plt.title('Distribution of Target Variable')
        plt.show()

    def missingValuesHeatmap(self):
